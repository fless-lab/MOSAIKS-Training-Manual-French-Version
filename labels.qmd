...
![Examples of 3 representative random convolutional features (downloaded from the MOSAIKS API) aggregated to different levels, including (A) grid cells (0.1°), (B) counties, and (C) states. The features are originally computed at 0.01° resolution (not pictured).](images/US_feature_summaries.png){#fig-feature-aggregation}

### API Features 

Currently the MOSAIKS API has a single global set of features that are precalculated and ready to download. This static layer is computed from Planet Labs, Inc. [Basemaps](https://developers.planet.com/docs/basemaps/) product Global Quarterly 2019 (qaurter 3) which has a native resolution of 4.77 m in the red, green, and blue bands of the visual spectrum (see @sec-satellite-private for more details). The features are publicly available for download; this is the fastest and easiest way to begin using MOSAIKS. 
...
In the next chapter, we'll explore specific examples of labels that work well with MOSAIKS.
:::
