...
MOSAIKS est conçu pour fonctionner "hors de la boîte" pour une large gamme d'applications SIML, pour les personnes sans expertise en SIML qui travaillent sur des ordinateurs de bureau ou des ordinateurs portables normaux. Pour de nombreuses applications, les utilisateurs de MOSAIKS n'ont jamais besoin de toucher à l'imagerie satellite eux-mêmes et n'ont besoin que d'une formation statistique de base.
...
1. Exécuter une régression linéaire avec validation croisée de vos étiquettes sur les fonctionnalités MOSAIKS (**Y = Xβ + e** ; ou tout autre modèle que vous choisissez ! Voir @sec-sec-model-choice).
