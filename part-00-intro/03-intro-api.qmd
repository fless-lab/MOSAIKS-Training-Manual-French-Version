...
::: {.callout-note}
This book is developed to proivide you with all the information you need to use MOSAIKS.
:::
...
The API features use input imagery from Planet Labs, Inc. [Visual Basemaps](https://developers.planet.com/docs/basemaps/) Global Quarterly 2019 (qaurter 3) product. Image quality, and therfore  feature quality, may be affected by local conditions. For example, an area undergoing a rainy season in the third quarter (July to September), is liklely to contain image artifacts from cloud cover. This will in turn effect the feature calculations. For more details on the input imagery @sec-satellite-imagery.
...
#### Agreggated feature files

Many users may find it easier to work with features aggregated to some level. The MOSAIKS API offers pre-aggregated features to accomodate these users need. Features come at several levels, including larger grid cells (0.1 and 1 degree) or summarized to administrative boundaries (ADM0, ADM1, andd ADM2). These files are available for download as either single or chunked files depending on the resolution.

These pre-aggregated features are particulalry useful for users in a few scenarios:
